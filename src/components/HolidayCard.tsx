  ...
  return (
    <div className="w-full glass-card rounded-2xl p-4 my-4 flex items-center justify-between relative overflow-hidden group border-amber-500/20 transition-transform duration-200 active:scale-95">
       ...
       <div className="flex items-center gap-4 relative z-10">
           <div className={`p-3 rounded-full shadow-inner ${isToday ? 'bg-amber-500 text-white animate-pulse' : 'bg-white/10 text-amber-500'}`}>
               {isToday ? <PartyPopper size={24} /> : <Gift size={24} />}
           </div>
           
           <div className="flex flex-col">
               <span className={`text-[10px] font-bold uppercase tracking-widest mb-0.5 ${isToday ? 'text-amber-500' : 'text-slate-400'}`}>
