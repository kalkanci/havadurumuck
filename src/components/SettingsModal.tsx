...
import { X, Settings, Vibrate, Thermometer, Wind, Download } from 'lucide-react';
      ...
      <div className={`relative w-full max-w-sm bg-slate-950/95 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl overflow-hidden ${isClosing ? 'animate-pop-out' : 'animate-pop-in'}`}>
        ...
        <div className="space-y-4">
            ...
            {onInstall && (
                <button
                    onClick={onInstall}
                    className="w-full bg-blue-600/20 p-4 rounded-2xl border border-blue-500/30 flex items-center justify-between group active:scale-[0.98] transition-all hover:bg-blue-600/30"
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                            <span className="block text-blue-200 font-bold text-sm">Uygulamayı Yükle</span>
                            <span className="text-xs text-blue-400/80">Ana ekrana ekleyin</span>
                        </div>
                    </div>
                </button>
            )}
            ...
            >
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg transition-colors ${settings.windSpeedUnit === 'kmh' ? 'bg-teal-500/20 text-teal-400' : 'bg-purple-500/20 text-purple-400'}`}>
                        <Wind size={18} />
                    </div>
                    <div className="text-left">
                        <span className="block text-slate-200 font-bold text-sm">Rüzgar Hızı Birimi</span>
                        <span className="text-xs text-slate-500">{settings.windSpeedUnit === 'kmh' ? 'km/h' : 'mph'}</span>
                    </div>
                </div>
