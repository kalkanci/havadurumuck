...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onUpdate }) => {
  ...
  const toggleWindSpeedUnit = () => {
    onUpdate({ ...settings, windSpeedUnit: settings.windSpeedUnit === 'mph' ? 'kmh' : 'mph' });
  };
    ...
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-6">
      ...
      <div className={`relative w-full max-w-sm bg-slate-950/95 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl overflow-hidden ${isClosing ? 'animate-pop-out' : 'animate-pop-in'}`}>
        ...
        <div className="space-y-4">
            ...
            >
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg transition-colors ${settings.windSpeedUnit === 'mph' ? 'bg-purple-500/20 text-purple-400' : 'bg-teal-500/20 text-teal-400'}`}>
                        <Wind size={18} />
                    </div>
                    <div className="text-left">
                        <span className="block text-slate-200 font-bold text-sm">Rüzgar Birimi</span>
                        <span className="text-xs text-slate-500">{settings.windSpeedUnit === 'mph' ? 'Mil/Saat (mph)' : 'Kilometre/Saat (km/h)'}</span>
                    </div>
                </div>

                <div className="px-3 py-1 bg-white/5 rounded-lg border border-white/5 text-xs font-bold text-slate-300">
                    {settings.windSpeedUnit === 'mph' ? 'mph' : 'km/h'}
                </div>
            </button>
        </div>
