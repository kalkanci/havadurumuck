...
const DetailsGrid: React.FC<DetailsGridProps> = ({ weather, unit, windSpeedUnit }) => {
  ...
  const speedUnit = windSpeedUnit === 'kmh' ? 'km/s' : 'mph';
                      ...
                      <div key={i} className="flex items-center justify-between py-1.5 border-b border-white/5 last:border-0">
                          ...
                          <div className="flex-1 px-4">
                               <div className="w-full h-1.5 bg-slate-700/50 rounded-full overflow-hidden">
                                   <div className="h-full bg-teal-500 rounded-full shadow-[0_0_8px_rgba(20,184,166,0.5)]" style={{ width: `${Math.min(h.windSpeed * (windSpeedUnit === 'mph' ? 5 : 3), 100)}%` }}></div>
                               </div>
                          </div>
      ...
      <div className="flex flex-col h-full gap-6">
          <div className="flex items-center justify-center gap-8 py-4 shrink-0">
              ...
              <div className="text-center">
                  ...
                  <p className="text-4xl font-black text-cyan-400 drop-shadow-lg">{Math.round(convertTemperature(current.dew_point_2m, unit))}{tempUnit}</p>
              </div>
          </div>
          ...
          <div className="bg-blue-500/10 p-5 rounded-2xl border border-blue-500/20 text-center shrink-0">
              <p className="text-blue-200 text-sm font-medium leading-relaxed">
                  Çiy noktası <span className="text-white font-bold">{Math.round(convertTemperature(current.dew_point_2m, unit))}{tempUnit}</span> seviyesinde. Hissedilen hava şuan <span className="font-bold text-white underline decoration-blue-400/50">{getDewPointDesc(current.dew_point_2m)}</span>.
              </p>
          </div>
  ...
  const items = [
    {
      id: 'feels_like',
      label: 'Hissedilen',
      value: Math.round(convertTemperature(current.apparent_temperature, unit)),
      unit: tempUnit,
      icon: <Thermometer size={24} className="text-red-400" />,
      subtext: null,
      render: () => {
          const feels = Math.round(convertTemperature(current.apparent_temperature, unit));
          const actual = Math.round(convertTemperature(current.temperature_2m, unit));
          return renderBasicContent(
              feels,
              tempUnit,
              feels > actual ? 'Olduğundan daha sıcak' : 'Olduğundan daha soğuk',
              `Rüzgar ve nem faktörleri hesaba katıldığında sıcaklık ${feels}${tempUnit} olarak hissediliyor.`
          );
      }
    },
    {
    ...
    },
    {
      id: 'dew_point',
      label: 'Çiy Noktası',
      value: Math.round(convertTemperature(current.dew_point_2m, unit)),
      unit: tempUnit,
