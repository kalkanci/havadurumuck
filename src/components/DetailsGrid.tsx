  ...
  const items = [
    {
      ...
      render: () => renderBasicContent(
          Math.round(convertTemperature(current.apparent_temperature, unit)),
          tempUnit, 
          current.apparent_temperature > current.temperature_2m ? 'Olduğundan daha sıcak' : 'Olduğundan daha soğuk',
          `Rüzgar ve nem faktörleri hesaba katıldığında sıcaklık ${Math.round(convertTemperature(current.apparent_temperature, unit))}${tempUnit} olarak hissediliyor.`
      )
    },
    {
      id: 'wind',
