      ...
      <div className="flex flex-col h-full gap-6">
          <div className="flex items-center justify-center gap-8 py-4 shrink-0">
              ...
              <div className="text-center">
                  ...
                  <p className="text-4xl font-black text-cyan-400 drop-shadow-lg">{convertTemperature(current.dew_point_2m, unit)}{tempUnit}</p>
              </div>
          </div>
          ...
          <div className="bg-blue-500/10 p-5 rounded-2xl border border-blue-500/20 text-center shrink-0">
              <p className="text-blue-200 text-sm font-medium leading-relaxed">
                  Çiy noktası <span className="text-white font-bold">{convertTemperature(current.dew_point_2m, unit)}{tempUnit}</span> seviyesinde. Hissedilen hava şuan <span className="font-bold text-white underline decoration-blue-400/50">{getDewPointDesc(current.dew_point_2m)}</span>.
              </p>
          </div>
  ...
  const items = [
    {
      id: 'feels_like',
      label: 'Hissedilen',
      value: convertTemperature(current.apparent_temperature, unit),
      unit: tempUnit,
      icon: <Thermometer size={24} className="text-red-400" />,
      subtext: null,
      render: () => renderBasicContent(
          convertTemperature(current.apparent_temperature, unit),
          tempUnit, 
          current.apparent_temperature > current.temperature_2m ? 'Olduğundan daha sıcak' : 'Olduğundan daha soğuk',
          `Rüzgar ve nem faktörleri hesaba katıldığında sıcaklık ${convertTemperature(current.apparent_temperature, unit)}${tempUnit} olarak hissediliyor.`
      )
    },
    {
      id: 'wind',
    ...
    },
    {
      id: 'dew_point',
      label: 'Çiy Noktası',
      value: convertTemperature(current.dew_point_2m, unit),
      unit: tempUnit,
