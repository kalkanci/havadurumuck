...
import { getWindDirection, formatTime, triggerHapticFeedback, convertTemp } from '../utils/helpers';
...
const DailyForecast: React.FC<DailyForecastProps> = ({ weather, unit }) => {
  ...
  const globalMinVal = Math.min(...temperature_2m_min);
  const globalMaxVal = Math.max(...temperature_2m_max);
  const displayedGlobalMin = convertTemp(globalMinVal, unit);
  const displayedGlobalMax = convertTemp(globalMaxVal, unit);
  const displayedRange = displayedGlobalMax - displayedGlobalMin;
  
  const hottestDayIndex = temperature_2m_max.indexOf(globalMaxVal);
  const coldestDayIndex = temperature_2m_min.indexOf(globalMinVal);
  ...
  const renderDetailModal = () => {
      ...
      const maxT = Math.round(convertTemp(temperature_2m_max[index], unit));
      const minT = Math.round(convertTemp(temperature_2m_min[index], unit));
      const feelsVal = apparent_temperature_max ? apparent_temperature_max[index] : temperature_2m_max[index];
      const feels = Math.round(convertTemp(feelsVal, unit));
    ...
    <div className="w-full flex flex-col pb-24 animate-fade-in-up">
            ...
            <div className={`w-full rounded-3xl p-6 bg-gradient-to-br ${trendColor} border border-white/10 shadow-2xl relative overflow-hidden mb-8`}>
                ...
                <div className="relative z-10 grid grid-cols-3 gap-3">
                    <div className="bg-black/20 rounded-2xl p-3 backdrop-blur-sm border border-white/5 flex flex-col items-center justify-center text-center">
                        ...
                        <span className="text-xl font-black text-white">{Math.round(displayedGlobalMax)}{tempUnit}</span>
                    ...
                    <div className="bg-black/20 rounded-2xl p-3 backdrop-blur-sm border border-white/5 flex flex-col items-center justify-center text-center">
                        ...
                        <span className="text-xl font-black text-white">{Math.round(displayedGlobalMin)}{tempUnit}</span>
        ...
        <div className="space-y-3 px-1">
            {time.map((dateStr, index) => {
                ...
                const minVal = temperature_2m_min[index];
                const maxVal = temperature_2m_max[index];
                const min = Math.round(convertTemp(minVal, unit));
                const max = Math.round(convertTemp(maxVal, unit));

                const rainProb = precipitation_probability_max ? precipitation_probability_max[index] : 0;
                const code = weather_code[index];
                
                // Calculate visualization bars (using displayed values for consistency)
                const leftPos = ((min - displayedGlobalMin) / displayedRange) * 100;
                const width = ((max - min) / displayedRange) * 100;
