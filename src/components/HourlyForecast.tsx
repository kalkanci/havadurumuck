...
const HourlyForecast: React.FC<HourlyForecastProps> = ({ weather, unit, windSpeedUnit }) => {
  ...
  const renderModal = () => {
    ...
    const temp = convertTemperature(tempRaw, unit);
    ...
    const feelsLikeRaw = weather.hourly.apparent_temperature?.[index] ?? tempRaw;
    ...
    const windSpeed = convertWindSpeed(weather.hourly.wind_speed_10m?.[index] ?? 0, windSpeedUnit);
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-6">
            ...
            <div className={`relative w-full max-w-sm bg-slate-900/90 border border-white/10 rounded-[2.5rem] p-8 shadow-2xl flex flex-col items-center overflow-hidden backdrop-blur-xl ${isClosing ? 'animate-pop-out' : 'animate-pop-in'}`}>
                 ...
                 <div className="relative z-10 flex flex-col items-center">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 bg-white/5 px-3 py-1 rounded-full border border-white/5">{formatTime(time)}</span>
      ...
      <div className="overflow-x-auto no-scrollbar py-4 -my-4 px-1 snap-x snap-mandatory scroll-pl-4">
        <div className="flex space-x-3 min-w-max pb-2">
          {forecastIndices.map((dataIndex, displayIndex) => {
            ...
            const tempRaw = weather.hourly.temperature_2m[dataIndex];
            const temp = convertTemperature(tempRaw, unit);
            ...
            // Color logic still uses raw celsius value for consistency
            const tempColor = getTempColor(tempRaw, isDay);
              ...
              <button 
                ...
                className={`
                  ...
                  ${isNow 
                    ...
                    : 'glass-card hover:bg-white/10 shadow-lg'}
                `}
              >
                {/* Time */}
