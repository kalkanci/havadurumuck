...
import { formatTime, triggerHapticFeedback, convertTemp } from '../utils/helpers';
...
const HourlyForecast: React.FC<HourlyForecastProps> = ({ weather, unit }) => {
  ...
  const renderModal = () => {
    ...
    const tempVal = weather.hourly.temperature_2m[index];
    const temp = convertTemp(tempVal, unit);
    ...
    const feelsLikeVal = weather.hourly.apparent_temperature?.[index] ?? tempVal;
    const feelsLike = convertTemp(feelsLikeVal, unit);
      ...
      <div className="overflow-x-auto no-scrollbar py-4 -my-4 px-1 snap-x snap-mandatory scroll-pl-4">
        <div className="flex space-x-3 min-w-max pb-2">
          {forecastIndices.map((dataIndex, displayIndex) => {
            ...
            const tempVal = weather.hourly.temperature_2m[dataIndex];
            const temp = convertTemp(tempVal, unit);
            ...
            const tempColor = getTempColor(tempVal, isDay); // Color logic uses raw Celsius
