...
export const generateSmartAdvice = (weather: WeatherData, unit: 'celsius' | 'fahrenheit' = 'celsius'): AdviceResponse => {
  ...
  // Aşırı Sıcak (>32°C)
     ...
     mood = "Kavurucu";
     advice = `Güneş yakıcı seviyede (${Math.round(displayTemp)}°${unit === 'fahrenheit' ? 'F' : 'C'}). Dışarıda fazla kalmamaya ve bol sıvı tüketmeye dikkat et.`;
     activities = ["Yüzme Havuzu", "Kliması Olan Mekanlar", "Soğuk Kahve Molası", "Siesta"];
  } 
  // Soğuk (<5°C)
  else if (temp < 5) { 
     mood = "Dondurucu";
     advice = `Hava ısırıyor (${Math.round(displayTemp)}°${unit === 'fahrenheit' ? 'F' : 'C'})! Kat kat giyinmeden kapıdan çıkma. Sıcak içecekler en iyi dostun olacak.`;
...
export const checkWeatherAlerts = (weather: WeatherData, unit: 'celsius' | 'fahrenheit' = 'celsius'): WeatherAlert[] => {
    ...
    if (current.temperature_2m > 35) {
        alerts.push({
            ...
            level: 'warning',
            title: 'Aşırı Sıcak',
            message: `Sıcaklık ${Math.round(convertTemperature(current.temperature_2m, unit))}°${unit === 'fahrenheit' ? 'F' : 'C'}'ye ulaştı. Bol su tüketin ve güneşten korunun.`
        });
    }
    ...
    if (current.temperature_2m < 0) {
        alerts.push({
            ...
            level: 'warning',
            title: 'Don Tehlikesi',
            message: `Sıcaklık sıfırın altında (${Math.round(convertTemperature(current.temperature_2m, unit))}°${unit === 'fahrenheit' ? 'F' : 'C'}). Buzlanmaya karşı dikkatli olun ve sıkı giyinin.`
        });
    }
