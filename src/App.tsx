...
import { Heart, Navigation, MapPin, ArrowUp, ArrowDown, RefreshCw, Calendar, CloudSun, Loader2, Settings, Trophy } from 'lucide-react';
    ...
    >
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-[320px]">
        <div className="relative flex items-center bg-black/40 backdrop-blur-2xl border border-white/10 rounded-full p-1.5 shadow-2xl">
          <div 
            className={`absolute top-1.5 bottom-1.5 w-[calc(33.33%-4px)] rounded-full bg-blue-500 shadow-lg shadow-blue-500/20 transition-all duration-300 cubic-bezier(0.4, 0, 0.2, 1) ${
              activeTab === 'today' ? 'left-1.5' : 'left-[calc(33.33%+2px)]'
            }`}
          />
          ...
          >
              ...
              <span className="hidden sm:inline">Uzun Vade</span>
          </button>

           <a
            href="https://futbol-tahmin-mvp.vercel.app"
            target="_blank"
            rel="noopener noreferrer"
            aria-label="Futbol Tahminleri"
            className="relative z-10 flex-1 flex items-center justify-center gap-2 py-3 rounded-full text-sm font-bold text-white/60 hover:text-green-400 transition-colors duration-300"
          >
              <Trophy size={18} />
