...
import { Heart, Navigation, RefreshCw, Calendar, CloudSun, Loader2, Settings, WifiOff } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const { refreshing, handleTouchStart, handleTouchMove, handleTouchEnd } = usePullToRefresh({
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col min-h-screen px-4 pb-24 max-w-md mx-auto w-full transition-transform duration-300">
        ...
        {/* Offline Indicator */}
        {isOffline && (
            <div className="absolute top-0 left-0 right-0 bg-red-500/80 backdrop-blur-sm text-white text-xs font-bold text-center py-1 z-[60]">
        ...
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-red-400 text-lg mb-2 font-bold">
                {error.code === 'NETWORK' ? 'Bağlantı Sorunu' :
                 error.code === 'GPS' ? 'Konum Hatası' : 'Bir Hata Oluştu'}
            </p>
            <p className="text-zinc-300 max-w-xs mx-auto mb-6">{error.message}</p>
            <button 
              onClick={() => refreshAll()}
