...
export const fetchAstronomyPicture = async (): Promise<AstronomyData | null> => {
  try {
    const response = await fetchWithRetry(`${NASA_API_URL}?api_key=${API_KEY}`, {}, 1, 500);
    ...
    if (error instanceof ApiError) {
        console.warn(`NASA API Error: ${error.status}. Using fallback.`);
    } else if (error instanceof NetworkError) {
        console.warn(`NASA Network Error: ${error.message}. Using fallback.`);
    } else {
