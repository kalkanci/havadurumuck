...
export const fetchAstronomyPicture = async (): Promise<AstronomyData | null> => {
  try {
    const response = await fetchWithRetry(`${NASA_API_URL}?api_key=${API_KEY}`, {}, 1, 500); // reduced retries for astronomy fallback
    ...
    if (error instanceof ApiError || error instanceof NetworkError) {
        console.warn(`Astronomy API error: ${error.message}. Using fallback.`);
    } else {
