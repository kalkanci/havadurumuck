...
export const useWeatherApp = () => {
  ...
  }, [location, initialBoot, haptic, loadHolidays, loadAstronomy]);
  ...
  const shareWeather = useCallback(async (): Promise<boolean> => {
      if (!weather || !location) return false;

      const temp = Math.round(weather.current.temperature_2m);
      const condition = getWeatherLabel(weather.current.weather_code);
      const feel = Math.round(weather.current.apparent_temperature);

      const text = `${location.name}: ${temp}°C, ${condition}. Hissedilen: ${feel}°C.\nAtmosferAI ile gönderildi.`;

      if (navigator.share) {
          try {
              await navigator.share({
                  title: `Hava Durumu: ${location.name}`,
                  text: text,
              });
              haptic(50);
              return true;
