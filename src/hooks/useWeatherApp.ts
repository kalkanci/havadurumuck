      ...
      if (err instanceof AppError) {
          ...
          switch(err.code) {
              case ErrorCode.NETWORK_ERROR:
                  setError('İnternet bağlantısı yok. Lütfen kontrol edin.');
                  break;
              case ErrorCode.API_ERROR:
                  setError('Hava durumu servisine ulaşılamıyor (Sunucu Hatası).');
                  break;
              case ErrorCode.RATE_LIMIT:
                  setError('Çok fazla istek yapıldı. Lütfen biraz bekleyin.');
                  break;
              case ErrorCode.INVALID_DATA:
                  setError('Konum verisi geçersiz.');
                  break;
              default:
                  setError(`Hata: ${err.message}`);
          }
      } else {
          setError('Hava durumu verisi alınamadı. Beklenmedik bir hata oluştu.');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
            ...
            // Even if address lookup fails, we have coords. But getDetailedAddress handles its own errors usually.
            // If it throws AppError (which we changed it to potentially do?), we handle it.
            // But we changed getDetailedAddress to return fallback on error, so it shouldn't throw unless critical.
        }
      },
      (err) => {
