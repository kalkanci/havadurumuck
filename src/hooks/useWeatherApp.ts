...
export const useWeatherApp = () => {
  ...
  const loadWeather = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (err instanceof AppError) {
          ...
          setError(new AppError('Hava durumu verisi alınamadı.', ErrorCode.UNKNOWN_ERROR));
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleCurrentLocation = useCallback(() => {
    ...
    if (!navigator.geolocation) {
      handleLocationError(new AppError('Cihazınız konum özelliğini desteklemiyor.', ErrorCode.LOCATION_DENIED));
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
            ...
            handleLocationError(new AppError('Konum adresi çözümlenemedi.', ErrorCode.GPS_ERROR));
        }
      },
      (err) => {
        console.warn("GPS Error:", err);
        handleLocationError(new AppError('Konum alınamadı.', ErrorCode.GPS_ERROR));
      },
      {
  ...
  const handleLocationError = useCallback((err: AppError) => {
      ...
      if (initialBoot) {
          ...
          setGpsError(true);
          // Still set error so UI knows? Maybe not for initial boot fallback
      } else {
          setError(err);
          setLoading(false);
          setGpsError(true);
